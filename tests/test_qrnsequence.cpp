...
const char testProgramParameters[] =
   "[OPTIONS] seq SEQ      [[START] END]\n"
   "       test_qrnsequence [OPTIONS] net NET SIZE [[START] END]";
const char testProgramUsage[] =
   "Prints the point set generated by a QRN sequences.\n\n";
const char testProgramName[] = "test_qrnsequence";
...
void test (int argc, char** argv)
{
   ...
   try
   {
      ...
      if (type == NET)
      {
         if (argc < 3)  usage("Invalid number of arguments!");
         S = atoi (argv [2]);
         next = 3;
      ...
      if (argc - next > 2)  usage("Too many arguments!");
      ...
      if (type == NET && END > SIZE)
      {
         usage("END is beyond the size of the net!");
      }

      if (START >= END)  usage("START is larger than END!");
      ...
      NORMAL
      {
         ...
         if (type == SEQUENCE)
         {
            cout << "Printing QRNSequence ";
            doubleQuote (cout, L::Make::getQrnSequenceName (SEQ));
            cout << " in dimension s = " << DIM << ".\n\n";
         }
         else
         {
            cout << "Printing QRNNet ";
            doubleQuote (cout, L::Make::getQrnNetName (SEQ));
            cout << " in dimension s = " << DIM
                 << " with " << SIZE << " points.\n\n";
         }
      }
