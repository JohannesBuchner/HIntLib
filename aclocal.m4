...
case $host_os in
  ...
  if AC_TRY_EVAL(ac_compile); then
   if test "$lt_cv_prog_gnu_ld" = yes; then
    case `/usr/bin/file conftest.$ac_objext` in
    *32-bit*)
      hl_cxx_abi_flag="-melf32bsmip"
      ;;
    *N32*)
      hl_cxx_abi_flag="-melf32bmipn32"
      ;;
    *64-bit*)
      hl_cxx_abi_flag="-melf64bmip"
      ;;
    esac
   else
    case `/usr/bin/file conftest.$ac_objext` in
  ...
  rm -rf conftest*
	_LT_AC_TAGVAR(archive_cmds, $1)='$CC '$hl_cxx_abi_flag' -shared -all -multigot $predep_objects $libobjs $deplibs $postdep_objects $compiler_flags -soname $soname `test -n "$verstring" && echo -set_version $verstring` -update_registry ${objdir}/so_locations -o $lib'

	# Archives containing C++ object files must be created using
	# "CC -ar", where "CC" is the IRIX C++ compiler.  This is
	# necessary to make sure instantiated templates are included
	# in the archive.
	_LT_AC_TAGVAR(old_archive_cmds, $1)='$CC '$hl_cxx_abi_flag' -ar -WR,-u -o $oldlib $oldobjs'
	;;
      *)
	if test "$GXX" = yes; then
	  if test "$with_gnu_ld" = no; then
  ...
  solaris*)
	...
	if test "$GXX" = yes && test "$with_gnu_ld" = no; then
	  ...
	  if $CC --version | grep -v '^2\.7' > /dev/null; then
	    ...
	    # what "hidden" libraries, object files and flags are used when
	    # linking a shared library.
	    output_verbose_link_cmd="$CXX -shared $CXXFLAGS -v conftest.$objext 2>&1 | grep \"\-L\""
	  else
	    ...
	    # what "hidden" libraries, object files and flags are used when
	    # linking a shared library.
	    output_verbose_link_cmd="$CXX -G $CFLAGS -v conftest.$objext 2>&1 | grep \"\-L\""
	  fi
